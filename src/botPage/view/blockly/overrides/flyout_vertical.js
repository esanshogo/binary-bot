...
Blockly.VerticalFlyout.prototype.position = function() {
    ...
    this.height_ = targetWorkspaceMetrics.viewHeight - 20; // binary-bot: -20 so it's separated from toolbox.
    ...
    if (this.targetWorkspace_.toolboxPosition == this.toolboxPosition_) {
        ...
        if (targetWorkspaceMetrics.toolboxWidth) {
            if (this.toolboxPosition_ == Blockly.TOOLBOX_AT_LEFT) {
                var x = targetWorkspaceMetrics.toolboxWidth - 100; // binary-bot: Dirty spacing, but it works.
            } else {
